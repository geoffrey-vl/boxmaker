...
def t_slot(center, orient, screw_diameter, nut_diameter, nut_height):
  ...
  #path.append_from_last(-up * (nut_r - 0*screw_r))
  #path.append_from_last(out * (screw_length - thickness - orient.norm() - nut_height))
...
def t_slots((rx,ry),(sox,soy),(eox,eoy),tabVec,length,(dirx,diry),isTab, do_holes):
  ...
  orient = Vec2([-diry * (screw_length - thickness - nut_height), dirx * (screw_length - thickness - nut_height)])
