...
def t_slot(center, orient, screw_diameter, nut_diameter, nut_height):
    '''
...
def t_slots((rx,ry),(sox,soy),(eox,eoy),tabVec,length,(dirx,diry),isTab, do_holes):
    ...
    orient = Vec2([-diry * (screw_length - thickness ), dirx * (screw_length - thickness )])
    ...
    center = Vec2(Vx + dirx* (gapWidth + tabWidth/2.),
            Vy + diry* (gapWidth + tabWidth/2.)) + (orient / orient.norm()) * thickness
    slot = t_slot(center, orient, screw_diameter, nut_diameter, nut_height)
...
def side((rx,ry),(sox,soy),(eox,eoy),tabVec,length,(dirx,diry),isTab):
    #       root startOffset endOffset tabVec length  direction  isTab
#    if DEBUG :
#        inkex.errormsg(_('side {}'.format(  isTab )))
...
#Front panel 
#        self.OptionParser.add_option('--front_string',action='store',type='string',
#                dest='front_string',default='NHNHNHNH',help='edge format string')
...
#Bottom panel 
    ...
    def effect(self):
        global parent,nomTab,equalTabs,thickness,correction, screw_length, screw_diameter, nut_diameter, nut_height, spacing, Z
